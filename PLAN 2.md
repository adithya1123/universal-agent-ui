...
| Auth | Databricks PAT (dev) / OAuth (deployed) |
...
3. **MLflow ResponsesAgent adapter**
   ...
   - Auth: Bearer PAT token from agent config
...
5. **Agent registry CRUD**
   ...
   - Frontend: Agent registration form (name, endpoint URL, PAT token)
...
| OAuth token flow for deployed apps | `databricks auth token` → Bearer header |
